...
def index(request):
    ...
    # post送信時に処理
    # messages = None # messagesが無い場合がある
...
def add(request):
    ...
    print(public_user, public_group)
...
def good(request, good_id):
    ...
    # ゼロより大きければ既にgood済み
    if is_good > 0:
        messages.success(request, '既にメッセージには、goodしています。')
        return redirect(to='/sns')
    # Messageのgood_countを1増やす + 1
...
# 指定されたグループおよび検索文字によるMesssageの取得
def get_your_group_message(owner, glist, find):
    ...
    print(me_friends)
    # FriendsのUserをリストにまとめる
    me_users = []
    messages = None # messagesが無い場合があるので、None
    for f in me_friends:
        ...
        # groupがgroupsに含まれるか、me_groupsに含まれるMessegeの取得
        if find == None:
